...
sd_mf = abs(0.3 * 0.0131234)
q.arg = list(mean = 0.0131234, sd = sd_mf)
...
melt_plot <- ggplot(data = melt_df) +
 ...
 # scale_y_continuous(sec.axis = sec_axis(~., name = "flow")) +
  theme_classic() +
 ...
 scale_color_manual(labels = c("Melt", "Total SWE"), values = c("cornflowerblue", "lightblue")) +
...
bathtub_df <- as.data.frame(outflow(input_df = melt_df, storage_initial = 70555, k = 0.001, evap = 22.66, outflow_only = FALSE))
...
# distributions for parameters
 sobol_k = rnorm(mean = 0.001, sd = 0.2, n = np)
...
q.arg = list(list(mean = 0.001, sd = sd_k), list(mean = 22.66, sd = sd_evap))
...
bathtub_plot <- ggplot(data = bathtub_df) +
  geom_area(aes(x = date, y = storage_new, group = 1, color = "storage_new"), size = 1, fill = "lightblue") +
  geom_line(aes(x = date, y = flow, group = 1, color = "flow"), size = 0.5) +
  geom_line(aes(x = date, y = flow_out, group = 1, color = "flow_out"), size = 0.5)+
 ...
 scale_color_manual(labels = c("Daily Storage", 
                               "Flow In", 
                               "Flow Out"#, 
                               #"Spill"
                               ), 
  ...
  #scale_y_continuous(expand = c(0,0)) +
 # scale_x_date(expand = c(0,0)) +
  geom_hline(aes(yintercept = 70555),
             color = "black",
             linetype = "dashed",
             size = 1)
